...
daterange1 = [datetime(2016,3,1)+timedelta(x) for x in range(0,6)] # set timeframe for query here  
daterange2 = [datetime(2016,3,1)+timedelta(x) for x in range(5,11)]
daterange3 = [datetime(2016,3,1)+timedelta(x) for x in range(10,16)]
daterange4 = [datetime(2016,3,1)+timedelta(x) for x in range(15,21)]
daterange5 = [datetime(2016,3,1)+timedelta(x) for x in range(20,26)]
daterange6 = [datetime(2016,3,1)+timedelta(x) for x in range(25,32)]

dateranges = [[daterange1]+[daterange2]+[daterange3]+[daterange4]+[daterange5]+[daterange6]]
...
q = s.prepare('select gkg_id, url, mft_data, gcam_data, event_ids, event_themes, event_locations, event_actors, named_entities, tone_avg, source, source_location,  wordcount, themes from gkg_record_by_day where gkg_day = ?;')


pickles = [x for x in range(0,7)]
...
# bind the generic query to each particular argument and execute them concurrently, up to 32 at a time (in this case, will run all 6 days at once)
                        ...
                        results = execute_concurrent_with_args(s, q, args, concurrency=6) # tricky quirk of the cassandra driver -- must cast the date argument to a tuple, which NB requires the trailing comma!
...
# initialize a nested dictionary for output
        ...
        pickle.dump(data_dict, open('gkg_2016_mar.pkl{}'.format(p),'wb'))
