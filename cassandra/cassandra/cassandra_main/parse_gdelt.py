...
import zipfile, csv, sys, mimetypes, urllib2, os, re
...
CASSANDRA_CONTACT = '10.1.3.34' # fhopp: new contact?
EXPECTED_COLUMNS = {
    'gkg':27,
    'mentions':16,
    'event':61,
...
class EventMentionDate(Model):
    event_id = Text(primary_key=True)
    event_timestamp = DateTime(partition_key=True)
    event_timestamp.truncate_microseconds=True
    event_mention = Text()
    source_url = Text()
...
def sync_tables(host=CASSANDRA_CONTACT):
    ...
    sync_table(EventByDay)
    sync_table(EventByGkgId)
    sync_table(EventMentionDate)
...
def mention_parser(mention_data,infile_url,file_timestamp,infile_name,session,cluster):
   ...
   # update_gkg_day_query = session.prepare('UPDATE gkg_record_by_day SET event_ids=event_ids+? WHERE gkg_day=? AND url=?;')
    event_mention_create_query = session.prepare('INSERT INTO event_mention_date ({}) VALUES ({});'.format(','.join([k for k in EventMentionDate._columns.keys()]),','.join(['?' for k in EventMentionDate._columns.keys()])))
    ...
    for i,row in enumerate(mention_data):
        ...
        mention_dict = {
                'event_id':event_id,
                'event_timestamp':event_mention,
                'event_mention':mention_date,
                'source_url':gkg_record_url
        }
        #print(mention_dict.values())
        
        session.execute(event_mention_create_query, mention_dict)    
...
def event_parser(event_data,infile_url,file_timestamp,infile_name,session,cluster):
    ...
    for i,row in enumerate(event_data):
        ...
        source_url = row[60]
        date_added_timestamp = parse(row[59])
        date_added = date_added_timestamp.date()
